...
# 对于二分类模型，通常使用的是二元交叉熵损失函数nn.BCELoss（输入已经是sigmoid激活函数之后的结果）或者nn.BCEWithLogitsLoss(输入
# 尚未经过nn.Sigmoid激活函数）。
...
# 一般我们常用的是类的实现形式。它们封装在torch.nn模块之下，并且类名以Loss结尾。
# 常用的一些内置损失函数说明如下。
...
# nn.CrossEntropyLoss（交叉熵，用于多分类，要求label为稀疏编码，输入未经过nn.Softmax激活，对不平衡数据集可以用weights参数调整
# 类别权重）
# nn.NLLLoss（负对数似然损失，用于多分类，要求label为稀疏编码，输入经过nn.LogSoftmax激活）
# nn.CosineSimilarity（余弦相似度，可用于多分类）
# nn.AdaptiveLogSoftmaxWithLoss（一种适合非常多类且类别分布很不均衡的损失函数，会自适应地将多个小类别合成一个cluster）
# 更多损失函数的介绍参考如下知乎文章：《PyTorch的十八个损失函数》https://zhuanlan.zhihu.com/p/61379965
...
# 可见focal_loss让容易样本的权重衰减到原来的0.005/0.1054=0.00474
# 而让困难样本的权重只衰减到原来的0.0866/0.6931=0.12496
