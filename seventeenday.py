...
# 并增加许多炼丹的乐趣。
# TensorBoard正式这样一个神奇的炼丹可视化辅助工具。它原是TensorFlow的小弟，但它也能够很好地和Pytorch进行配合。甚至在Pytorch中使用
...
# 最后就可以传入日志目录作为参数启动TensorBoard，然后就可以在TensorBoard中愉快的看片来。
...
# 启动tensorboard程序
notebook.start("--logdir ./data/tesorboard")
...
# 也可以用torchvision.utils.make_grid将多张图片拼成一张图片，然后writer.add_image写入。
